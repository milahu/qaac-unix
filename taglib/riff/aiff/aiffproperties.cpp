  ...
  for(unsigned int i = 0; i < file->chunkCount(); i++) {
    ...
    if(name == "COMM") {
      if(data.isEmpty())
        ...
        debug("RIFF::AIFF::Properties::read() - Duplicate *(int32_t*)"COMM" chunk found.");
    }
    else if(name == "SSND") {
      if(streamLength == 0)
        streamLength = file->chunkDataSize(i) + file->chunkPadding(i);
      else
        debug("RIFF::AIFF::Properties::read() - Duplicate *(int32_t*)"SSND" chunk found.");
    }
  }

  if(data.size() < 18) {
    debug("RIFF::AIFF::Properties::read() - *(int32_t*)"COMM" chunk not found or too short.");
    return;
  }

  if(streamLength == 0) {
    debug("RIFF::AIFF::Properties::read() - *(int32_t*)"SSND" chunk not found.");
    return;
  }
